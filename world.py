            ...
            while cpos not in stops:
                ...
                for _ in range(3):
                    ...
                    for apos in cadjacents:
                        if self._hmap[apos[1]][apos[0]] <= self._hmap[lowest[1]][lowest[0]]:
                            lowest = apos
                ...
                while self._tmap[cpos[1]][cpos[0]] == "River":
                    if len(lowests) != 0:
            ...
            for x, tval in enumerate(row):
                if tval != "River":
                    ...
                    if adjacenttvals.count("River") >= smoothness:
                        self._tmap[y][x] = "River"
                        count += 1
        toc = t.perf_counter()
        if verbose:
            print(f"Done! ({toc -tic:0.4f} seconds and {count} smoothings)")

    def generate_terrain(self, rivers, river_distance, *, verbose=True):
        """
